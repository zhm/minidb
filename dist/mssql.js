...
class MSSQL extends _database.default {
  ...
  static set driver(driver) {
    mssql = driver; // PostgresConnection.driver = driver;
  }
  ...
  buildWhere(where) {
    ...
    if (where) {
      for (const key of Object.keys(where)) {
        ...
        if (value == null) {
          clause.push((0, _util.format)('[%s] IS NULL', key)); // } else if (Array.isArray(value)) {
  ...
  insertStatement(table, attributes, options) {
    ...
    const [names, placeholders, values] = this.buildInsert(attributes); // const returning = options && options.returnPrimaryKey === false ? '' : ' RETURNING ' + options.pk;
